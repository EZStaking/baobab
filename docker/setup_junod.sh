...
GENESIS_FILE="$HOME"/.baobab/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  baobabd init --chain-id "$CHAIN_ID" "$MONIKER"
  baobabd add-ica-config
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.baobab/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.baobab/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.baobab/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.baobab/config/config.toml
...
if ! baobabd keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | baobabd keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | baobabd add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    baobabd add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | baobabd gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | baobabd gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
  baobabd collect-gentxs
fi
