...
export HOME_DIR=$(eval echo "${HOME_DIR:-"~/.baobab/"}")

export BINARY=${BINARY:-baobabd}
...
baobabd config keyring-backend $KEYRING
baobabd config chain-id $CHAIN_ID
...
command -v baobabd > /dev/null 2>&1 || { echo >&2 "baobabd command not found. Ensure this is setup / properly installed in your GOPATH."; exit 1; }
...
from_scratch () {
  ...
  update_test_genesis '.app_state["staking"]["params"]["bond_denom"]="ubaobab"'
  update_test_genesis '.app_state["bank"]["params"]["send_enabled"]=[{"denom": "ubaobab","enabled": true}]'
  # update_test_genesis '.app_state["staking"]["params"]["min_commission_rate"]="0.100000000000000000"' # sdk 46 only

  update_test_genesis '.app_state["mint"]["params"]["mint_denom"]="ubaobab"'
  update_test_genesis '.app_state["gov"]["deposit_params"]["min_deposit"]=[{"denom": "ubaobab","amount": "1000000"}]'
  update_test_genesis '.app_state["crisis"]["constant_fee"]={"denom": "ubaobab","amount": "1000"}'

  update_test_genesis '.app_state["tokenfactory"]["params"]["denom_creation_fee"]=[{"denom":"ubaobab","amount":"100"}]'

  update_test_genesis '.app_state["feeshare"]["params"]["allowed_denoms"]=["ubaobab"]'

  # Allocate genesis accounts
  BINARY add-genesis-account $KEY 10000000ubaobab,1000utest --keyring-backend $KEYRING
  BINARY add-genesis-account feeacc 1000000ubaobab,1000utest --keyring-backend $KEYRING

  BINARY gentx $KEY 1000000ubaobab --keyring-backend $KEYRING --chain-id $CHAIN_ID
...
BINARY start --pruning=nothing  --minimum-gas-prices=0ubaobab --rpc.laddr="tcp://0.0.0.0:$RPC"
